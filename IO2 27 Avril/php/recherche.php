...
require_once 'database.php';
 ...
 $synop = $db->query('SELECT synopsis FROM animes ORDER BY id DESC');
 $img = $db->query('SELECT image FROM animes ORDER BY id DESC');
 if(isset($_POST['recherche']) AND !empty($_POST['recherche'])) {
    $recherche = htmlspecialchars($_POST['recherche']);
    $anime = $db->query('SELECT titre FROM animes WHERE titre LIKE "%'.$recherche.'%" ORDER BY id DESC');
    $synop = $db->query('SELECT synopsis FROM animes WHERE synopsis LIKE "%'.$recherche.'%" ORDER BY id DESC');
    $img = $db->query('SELECT image FROM animes WHERE image LIKE "%'.$recherche.'%" ORDER BY id DESC');
    if($anime->rowCount() == 0 AND $synop->rowCount() == 0) {
       ...
       $synop = $db->query('SELECT synopsis FROM animes WHERE CONCAT(titre, synopsis) LIKE "%'.$recherche.'%" ORDER BY id DESC');
       $img = $db->query('SELECT image FROM animes WHERE CONCAT(titre, synopsis) LIKE "%'.$recherche.'%" ORDER BY id DESC');
      }
 }
...
<body>
    ...
    <?php } elseif($anime->rowCount() > 0) { ?>
        <ul>
        <?php while($b = $synop->fetch() AND $a = $anime->fetch()) { ?>
            <br>
            <div class="active">
              <div class="supp">
            <center><h1 class="title"><?= $a['titre'] ?></h1></center>
            <br>
            <center><img src="../image/logo.png" width=20% height=40%></center>
            <br>
            <p class="time"><?= $b['synopsis'] ?></p>
            <br>
            </div>
        </div>
        <?php } ?>
